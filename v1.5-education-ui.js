...
function isIntroduced(item,k=knowledge()){if(!item)return false;const cards=state.readAppState().cards||{};return Boolean(cards[item.id]||k[item.character]?.exposedAt)}
function selectLearningItem(k,modes,excludeCharacters=[]){
  const all=deck().filter(item=>item?.character);
  const fresh=all.filter(item=>!isIntroduced(item,k)&&!excludeCharacters.includes(item.character));
  const pool=fresh.length?fresh:all.filter(item=>!excludeCharacters.includes(item.character));
...
async function publishV2LearningCard(){
  ...
  if(!boundary)return;
  const meanings=(edu.item.meaning||[]).join(' · ')||'—';
  const readings=[...(edu.item.on||[]),...(edu.item.kun||[])].join(' · ')||'—';
  await boundary.setExercise({mode:null,prompt:learningPrompt(),character:edu.item.character,stimulus:{kind:'learning',primary:edu.item.character,secondary:readings,translation:meanings,inputPlaceholder:''},answerHint:'',contentId:edu.item.id||edu.item.character,contentVersion:'1',provenance:'local'});
}
function renderLearning(){
  if(isV2())return;
  const p=pane(),item=edu.item;if(!p||!item)return;
  p.innerHTML=`<div class="v14-edu-wrap"><div class="v14-edu-title">🧠 کارت یادگیری</div><div class="v14-edu-prompt">${safe(learningPrompt())}</div><div class="v14-edu-kanji">${safe(item.character)}</div><div class="v14-edu-meaning">${safe((item.meaning||[]).join(' · ')||'—')}</div><div class="v14-edu-reading">${safe([...(item.on||[]),...(item.kun||[])].join(' · ')||'—')}</div><div class="v14-edu-actions"><button type="button" class="primary" id="v14EduStartExercise">شروع تمرین</button></div></div>`;
}
...
async function publishV2Exercise(){if(!isV2()||!edu.item||!edu.mode)return;const boundary=window.__KANJI5_V19_V2_BOUNDARY__;if(!boundary)return;let contentId=edu.item.id||edu.item.character,provenance='local',stimulus={kind:'kanji',primary:edu.item.character,inputPlaceholder:'Type your answer'},answerHint=answerText(),choices=[];if(edu.mode==='meaning'){stimulus={kind:'kanji',primary:edu.item.character,inputPlaceholder:'e.g. school'}}else if(edu.mode==='reading'){stimulus={kind:'kanji',primary:edu.item.character,inputPlaceholder:'e.g. gaku or がく'}}else if(edu.mode==='production'){stimulus={kind:'meaning',primary:(edu.item.meaning||[]).join(' · ')||'—',inputPlaceholder:''};choices=chooseChoices(edu.item)}else if(edu.mode==='vocabulary'&&edu.word){contentId=`${edu.item.character}:${edu.word.word}`;provenance='kanjiapi';stimulus={kind:'masked-vocabulary',primary:edu.word.word.replaceAll(edu.item.character,'＿'),secondary:edu.word.reading||'',translation:edu.word.meaning||'',inputPlaceholder:'Type the full word'};answerHint=`${edu.word.word} · ${edu.word.reading||''}`}else if(edu.mode==='context'&&edu.sentence){contentId=`${edu.item.character}:${edu.sentence.text}`;provenance='tatoeba';stimulus={kind:'masked-context',primary:edu.sentence.text.replaceAll(edu.item.character,'＿'),translation:edu.sentence.english||'',inputPlaceholder:''};answerHint=edu.item.character;choices=chooseChoices(edu.item)}await boundary.setExercise({mode:edu.mode,prompt:exercisePrompt(),character:edu.item.character,stimulus,choices,answerHint,contentId,contentVersion:'1',provenance})}
function renderResult(result){if(isV2())return;const p=pane();if(!p)return;const q=result.outcome==='unknown'?'نمی‌دانستم':result.quality==='partial'?'نسبی':result.quality==='exact'?'دقیق':result.outcome==='invalid'?'نامعتبر':'نادرست';p.innerHTML=`<div class="v14-edu-wrap"><div class="v14-edu-title">${result.outcome==='unknown'?'🟡 پاسخ را نمی‌دانستم':result.correct?'✅ پاسخ درست بود':'❌ پاسخ نادرست بود'}</div><div class="v14-edu-meta">${safe(q)} · ${stageLabel(result.stage||CORE.getStage(knowledge()[edu.item.character]))}</div><div class="v14-edu-answer">پاسخ صحیح: <strong>${answerText()}</strong></div><div class="v14-edu-actions"><button type="button" class="primary" id="v14EduNext">تمرین بعدی</button><button type="button" class="secondary" id="v14EduReview">بازگشت به مرور</button></div></div>`}
...
function render(){
  ...
  const p=pane(),item=edu.item;if(!p||!item)return;
  let prompt='',body='';
  if(edu.mode==='meaning'){prompt='معنی این کانجی را به انگلیسی بنویس.';body=`<div class="v14-edu-kanji">${safe(item.character)}</div><input id="v14EduInput" class="v14-edu-input" autocomplete="off" spellcheck="false" placeholder="مثلاً: school">`}
  else if(edu.mode==='reading'){prompt='یک خوانش رایج این کانجی را بنویس؛ Hiragana یا Romaji.';body=`<div class="v14-edu-kanji">${safe(item.character)}</div><input id="v14EduInput" class="v14-edu-input" autocomplete="off" spellcheck="false" placeholder="مثلاً: gaku یا がく">`}
  else if(edu.mode==='production'){prompt='با دیدن این معنی، کانجی مناسب را از چهار گزینه انتخاب کن.';body=`<div style="text-align:center;font-size:26px;font-weight:800;line-height:1.6">${safe((item.meaning||[]).join(' · ')||'—')}</div>${renderChoices(chooseChoices(item))}`}
  else if(edu.mode==='vocabulary'&&edu.word){prompt='واژهٔ کامل را به ژاپنی بنویس.';body=`<div class="v14-edu-word" style="font-size:30px;letter-spacing:0">${safe(edu.word.word.replaceAll(item.character,'＿'))}</div><div class="v14-edu-reading">${safe(edu.word.reading)}</div><div class="v14-edu-meaning">${safe(edu.word.meaning||'—')}</div><input id="v14EduVocabularyInput" class="v14-edu-input" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="واژه را وارد کن">`}
  else if(edu.mode==='context'&&edu.sentence){prompt='کانجیِ حذف‌شده را از چهار گزینه انتخاب کن.';const blanked=edu.sentence.text.replaceAll(item.character,'＿');body=`<div class="v14-edu-word" style="font-size:24px;line-height:1.8;letter-spacing:0">${safe(blanked)}</div><div class="v14-edu-meaning">${safe(edu.sentence.english||'—')}</div>${renderChoices(chooseChoices(item))}`}
  else{renderLearning();return}
  const check=(edu.mode==='meaning'||edu.mode==='reading'||edu.mode==='vocabulary')?'<button type="button" class="primary" id="v14EduSubmit">بررسی پاسخ</button>':'';
...
async function showLearningCard(){
  ...
  if(!item){edu={item:null,mode:null,word:null,sentence:null,answered:false};if(pane())pane().innerHTML='<div class="v14-edu-empty">در حال حاضر کانجی جدیدی برای آشنایی اولیه وجود ندارد.</div>';if(v2)await window.__KANJI5_V19_V2_BOUNDARY__?.clearTransient?.();return}
  state.writeKnowledge(CORE.ensureEntry(k,item.character,true));
  edu={item,mode:null,word:null,sentence:null,answered:false};
  if(v2)await publishV2LearningCard();else renderLearning();
}
async function startExercise(){
  if(!edu.item)return showLearningCard();
  const p=pane();if(p)p.innerHTML='<div class="v14-edu-empty"><div style="font-size:42px">⏳</div><strong>در حال آماده‌سازی تمرین…</strong></div>';
  const settings=state.readSettings(),modes=CORE.getAvailableModes({...DEFAULTS,...settings},true),k=knowledge(),stats=k[edu.item.character]||{};
  const adaptiveApi=await sessionApi();
  let mode=adaptiveApi?.nextMode?.(modes)||CORE.chooseBestExercise(edu.item,stats,modes,{now:Date.now()});
  let word=null,sentence=null,networkSatisfied=true;
  try{
    const api=await network();
    if(mode==='vocabulary'){
      const words=await api.fetchWords(edu.item.character);
      if(words.length)word=words[Math.floor(Math.random()*Math.min(words.length,8))];
      else{networkSatisfied=false;mode=CORE.chooseBestExercise(edu.item,stats,modes.filter(x=>x!=='vocabulary'),{now:Date.now()})}
    }else if(mode==='context'){
      const rows=await api.fetchContextSentences(edu.item.character);
      if(rows.length)sentence=rows[Math.floor(Math.random()*Math.min(rows.length,8))];
      else{networkSatisfied=false;mode=CORE.chooseBestExercise(edu.item,stats,modes.filter(x=>x!=='context'),{now:Date.now()})}
    }
  }catch(_){
    networkSatisfied=false;
    mode=CORE.chooseBestExercise(edu.item,stats,modes.filter(x=>x==='meaning'||x==='reading'||x==='production'||x==='context'),{now:Date.now()});
  }
  if(networkSatisfied)adaptiveApi?.consumeMode?.(mode);
  state.writeKnowledge(CORE.ensureEntry(k,edu.item.character,true));
  edu={...edu,mode,word,sentence,answered:false};
  if(isV2())await publishV2Exercise();else render();
}
...
async function recordOutcome(correct,wrong='',outcome=null){const normalized=outcome||(correct&&typeof correct==='object'?correct:null)||await normalizeOutcome(edu.mode,{correct:Boolean(correct),quality:correct?'exact':'wrong',score:correct?1:0});const next=CORE.recordKnowledge(knowledge(),edu.item.character,edu.mode,normalized.correct,wrong,state.deviceId());persistOutcome(next,edu.item.character,normalized);state.writeKnowledge(next);document.dispatchEvent(new CustomEvent('kanji5:v1.6-education-result',{detail:{mode:normalized.mode,correct:normalized.correct,outcome:normalized.outcome,quality:normalized.quality,score:normalized.score,schemaVersion:normalized.schemaVersion,graderVersion:normalized.graderVersion,character:edu.item.character}}));return next[edu.item.character]}
function record(correct,wrong=''){return recordOutcome(correct,wrong,correct&&typeof correct==='object'?correct:null)}
async function finish(result,wrong=''){if(edu.answered)return;const outcome=result.schemaVersion?result:await normalizeOutcome(edu.mode,result);if(outcome.outcome==='empty'||outcome.outcome==='invalid')return;edu.answered=true;const entry=await record(outcome,wrong);renderResult({...outcome,stage:entry.stage})}
async function submit(valueOverride=null){if(!edu.item||edu.answered)return;let result,wrong='';if(edu.mode==='meaning'){const value=valueOverride??($('#v14EduInput')?.value||'');result=CORE.gradeMeaning(value,edu.item.meaning||[]);if(result.quality==='empty'){$('#v14EduInput')?.focus();return}}else if(edu.mode==='reading'){const value=valueOverride??($('#v14EduInput')?.value||'');if(!CORE.normalize(value)){$('#v14EduInput')?.focus();return}result=CORE.gradeReading(value,[...(edu.item.on||[]),...(edu.item.kun||[])]);}else if(edu.mode==='production'){const value=valueOverride??($('#v14EduProductionInput')?.value||'');import('./v1.8-production-core.js').then(()=>{const api=globalThis.__KANJI5_V18_PRODUCTION__;result=api.gradeProduction(value,edu.item.character);if(result.quality==='empty'){$('#v14EduProductionInput')?.focus();return}void finish(result,'')}).catch(()=>{});return}else if(edu.mode==='vocabulary'){const value=valueOverride??($('#v14EduVocabularyInput')?.value||'');import('./v1.8-vocabulary-core.js').then(mod=>{result=mod.gradeVocabulary(value,edu.word?.word||'');if(result.quality==='empty'){$('#v14EduVocabularyInput')?.focus();return}void finish(result,'')}).catch(()=>{});return}else if(edu.mode==='context'){const value=valueOverride??($('#v14EduContextInput')?.value||'');import('./v1.8-context-core.js').then(mod=>{result=mod.gradeContext(value,edu.item.character);if(result.quality==='empty'){$('#v14EduContextInput')?.focus();return}void finish(result,'')}).catch(()=>{});return}else return;void finish(result,wrong)}
...
async function retry(){const api=window.__KANJI5_V19_RECOVERY__;if(!(await api?.retry?.()))return false;await startExercise();return true}
async function next(){edu={item:null,mode:null,word:null,sentence:null,answered:false};return showLearningCard()}
window.__KANJI5_EDU_BRIDGE__=Object.freeze({start,startExercise,submitValue,submitChoice:choose,dontKnow,retry,next,backToReview:()=>{location.href=location.pathname}});

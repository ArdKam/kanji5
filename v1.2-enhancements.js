(() => {
  ...
  let activePrompt = null, activeCharacter = null, deckIndex = null, originalRevealButton = null, allowNativeReveal = false;
  let adaptiveRecallPromise = null;
  function normalize(value){return String(value??"").trim().toLowerCase().normalize("NFKC").replace(/[\s\u3000]+/g,"")}
  function normalizeRomaji(value){return normalize(value).replace(/shi/g,"si").replace(/chi/g,"ti").replace(/tsu/g,"tu").replace(/fu/g,"hu").replace(/ji/g,"zi").replace(/ou/g,"o").replace(/aa/g,"a").replace(/ii/g,"i").replace(/uu/g,"u").replace(/ee/g,"e")}
  function kanaToRomaji(value){const map=new Map([["あ","a"],["い","i"],["う","u"],["え","e"],["お","o"],["か","ka"],["き","ki"],["く","ku"],["け","ke"],["こ","ko"],["さ","sa"],["し","shi"],["す","su"],["せ","se"],["そ","so"],["た","ta"],["ち","chi"],["つ","tsu"],["て","te"],["と","to"],["な","na"],["に","ni"],["ぬ","nu"],["ね","ne"],["の","no"],["は","ha"],["ひ","hi"],["ふ","fu"],["へ","he"],["ほ","ho"],["ま","ma"],["み","mi"],["む","mu"],["め","me"],["も","mo"],["や","ya"],["ゆ","yu"],["よ","yo"],["ら","ra"],["り","ri"],["る","ru"],["れ","re"],["ろ","ro"],["わ","wa"],["を","wo"],["ん","n"],["が","ga"],["ぎ","gi"],["ぐ","gu"],["げ","ge"],["ご","go"],["ざ","za"],["じ","ji"],["ず","zu"],["ぜ","ze"],["ぞ","zo"],["だ","da"],["ぢ","ji"],["づ","zu"],["で","de"],["ど","do"],["ば","ba"],["び","bi"],["ぶ","bu"],["べ","be"],["ぼ","bo"],["ぱ","pa"],["ぴ","pi"],["ぷ","pu"],["ぺ","pe"],["ぽ","po"]]);const digraphs={"きゃ":"kya","きゅ":"kyu","きょ":"kyo","しゃ":"sha","しゅ":"shu","しょ":"sho","ちゃ":"cha","ちゅ":"chu","ちょ":"cho","にゃ":"nya","にゅ":"nyu","にょ":"nyo","ひゃ":"hya","ひゅ":"hyu","ひょ":"hyo","みゃ":"mya","みゅ":"myu","みょ":"myo","りゃ":"rya","りゅ":"ryu","りょ":"ryo","ぎゃ":"gya","ぎゅ":"gyu","ぎょ":"gyo","じゃ":"ja","じゅ":"ju","じょ":"jo","びゃ":"bya","びゅ":"byu","びょ":"byo","ぴゃ":"pya","ぴゅ":"pyu","ぴょ":"pyo"};let out="";for(let i=0;i<value.length;i+=1){const pair=value.slice(i,i+2);if(digraphs[pair]){out+=digraphs[pair];i+=1;continue}if(value[i]==="っ"){out+=map.get(value[i+1])?.[0]||"";continue}if(value[i]==="ー")continue;out+=map.get(value[i])||value[i]}return out}
  ...
  async function loadAdaptiveRecall(){
    if(window.__KANJI5_V17_ADAPTIVE_RECALL__)return window.__KANJI5_V17_ADAPTIVE_RECALL__;
    if(!adaptiveRecallPromise)adaptiveRecallPromise=import('./v1.7-adaptive-recall-core.js').then(()=>window.__KANJI5_V17_ADAPTIVE_RECALL__||null).catch(()=>null);
    return adaptiveRecallPromise;
  }
  async function choosePrompt(character){
    const entry=loadKnowledge()[character]||{};
    try{
      const adaptive=await loadAdaptiveRecall();
      const selected=adaptive?.selectRecallAttributes?.({meaning:entry.meaning,reading:entry.reading},{maxAttributes:1,minScore:0.15});
      const mode=selected?.[0];
      if(mode==='meaning'||mode==='reading')return mode;
    }catch(_){ }
    return(Number(entry.reading?.attempts)||0)<(Number(entry.meaning?.attempts)||0)?"reading":"meaning";
  }
  ...
  function checkRecall(character,mode,value){const item=deckIndex?.get(character);if(!item)return null;const answer=normalize(value);if(!answer)return false;if(mode==="meaning")return gradeMeaningCanonical(value,item.meaning||[]);const answerRomaji=normalizeRomaji(answer);return[...(item.on||[]),...(item.kun||[])].some(reading=>{const kana=normalize(reading),romaji=normalizeRomaji(kanaToRomaji(kana));return answer===kana||(answerRomaji&&answerRomaji===romaji)})}

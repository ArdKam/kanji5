...
async function retry(){const current=active();if(!current)return false;const core=await loadCore();if(!core.canRetry(current))return false;return write(core.consumeRetry(current,new Date().toISOString()));}

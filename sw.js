...
const SHELL=["./","./index.html","./manifest.webmanifest","./icon.svg","./v1.3-p0.js","./v1.3-perf.js","./v1.3-storage-bridge.js","./v1.3-settings.js","./v1.4-education-migration.js","./v1.4-education-core.js","./v1.4-education-ui.js","./v1.5-state.js","./v1.5-recall-core.js","./v1.5-p0.js","./v1.5-education-sync-core.js","./v1.5-fsrs-sync-core.js","./v1.5-sync-core.js","./vendor/ts-fsrs-5.4.1.mjs","./v1.2-enhancements.js","./v1.2-runtime-fixes.js","./supabase-config.js","./supabase-sync.js"];
...
async function apiCacheFirst(req){const c=await caches.open(API_CACHE);const hit=await c.match(req);if(hit){const stamp=Number(hit.headers.get('X-Kanji5-Cache-Time')||0);if(stamp&&Date.now()-stamp<=API_TTL_MS){void evictApiCache(c);return hit}if(stamp)await c.delete(req)}const key=req.url;const pending=API_INFLIGHT.get(key);if(pending)return(await pending).clone();const request=(async()=>{try{const r=await fetch(req);await cacheApiResponse(c,req,r);void evictApiCache(c);return r}catch(_){const fallback=await c.match(req);return fallback||Response.error()}})();API_INFLIGHT.set(key,request);try{return(await request).clone()}finally{if(API_INFLIGHT.get(key)===request)API_INFLIGHT.delete(key)}}

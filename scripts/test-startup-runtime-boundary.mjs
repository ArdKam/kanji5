...
for (const file of legacyFiles) assert.ok(index.includes(`"${file.startsWith('./') ? file : './' + file}"`), `legacy compatibility loader lost ${file}`);
for (const file of legacyFiles) {
  const directTag = new RegExp(`<script[^>]+src="${file.replaceAll('.', '\\\.')}"[^>]*><\\/script>`);
  assert.doesNotMatch(index, directTag, `legacy file is directly wired into the default shell: ${file}`);
}

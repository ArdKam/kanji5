...
for (const file of legacyFiles) {
  assert.ok(index.includes(`"./${file}"`), `legacy compatibility loader lost ${file}`);
  assert.doesNotMatch(index, new RegExp(`<script[^>]+src="./${file.replaceAll('.', '\\\.')}"[^>]*><\\/script>`), `legacy file is directly wired into the default shell: ${file}`);
}
...
for (const file of [
  'app-bootstrap-v115.js','tmp.md','v1.3-settings.js','v1.3-perf.js',
  'v1.8-learning-ux.js','v1.9-recovery-ui.js'

...
for(const id of ['v16Session','v16Due','v16New','v16Mastered','v16Streak','v16GoalBar','v16Reviews','v16Recall','v16Unknown','v16Duration','v16Modes','v16Attention','v16Finish','v16CurrentSummary','v16History'])assert.match(session,new RegExp(`["']${id}["']`),`session dashboard node missing: ${id}`);
...
assert.match(state,/SESSION_HISTORY_KEY/,'state boundary must own session-history storage');
assert.match(state,/function appendSessionSummary\(/,'state boundary must expose session summary append');
assert.match(state,/SESSION_HISTORY_LIMIT=30/,'session history must remain bounded');

...
assert.match(session,/filter\(x=>x\?\.status===ACTIVE_STATUS&&String\(x\.experience\|\|'review'\)===session\.experience\)/,'active session persistence must be scoped to the current experience');
...
assert.match(sw,/const CACHE='kanji5-shell-v\d+'/,'service-worker cache must include the active versioned shell');
assert.match(sw,/"\.\/v1\.8-vocabulary-core\.js"/,'Vocabulary grader must be offline-precached');
assert.match(sw,/"\.\/v1\.8-context-core\.js"/,'Context grader must be offline-precached');

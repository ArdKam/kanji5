...
import {
  RECALL_MODES,
  applyAttributeRecallOutcome,
  attributeRecallAccuracy,
  attributeRecallSignal,
  normalize,
  selectWeakestRecallAttribute
...
assert.equal(attributeRecallAccuracy({ attempts: 0, correct: 0 }), 0);
assert.equal(attributeRecallAccuracy({ attempts: 2, correct: 2 }), 0.75);
assert.equal(attributeRecallAccuracy({ attempts: 4, correct: 1, score: 0.5 }), 0.25);

const item = { character: '学', meaning: ['study', 'learning'], on: ['ガク'], kun: ['まなぶ'] };
const focus = selectWeakestRecallAttribute(item, 'meaning', {
...
assert.equal(selectWeakestRecallAttribute(item, 'reading', {
  ガク: { attempts: 2, correct: 2 },
  まなぶ: { attempts: 1, correct: 0 }
})?.raw, 'まなぶ');
assert.equal(selectWeakestRecallAttribute(null, 'meaning'), null);
assert.equal(selectWeakestRecallAttribute(item, 'unsupported'), null);

const signal = attributeRecallSignal({
  meaning: {
...
const correct = applyAttributeRecallOutcome(original, 'meaning', 'study', 'correct', '2026-09-04T00:00:00.000Z');
...
const unknown = applyAttributeRecallOutcome(original, 'meaning', 'study', 'unknown', '2026-09-04T00:00:01.000Z');

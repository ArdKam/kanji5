...
assert(state.mode==='production'&&state.item?.character==='学',`Production mode did not start with selected Kanji; state=${JSON.stringify(state)}, pane=${panel.innerHTML}`);
...
assert(panel.innerHTML.includes('✅ پاسخ درست بود'),`Correct Production choice did not render a success result; pane=${panel.innerHTML}; answered=${api.getState().answered}`);

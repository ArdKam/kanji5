...
assert(api&&typeof api.startEducation==='function'&&typeof api.handleChoice==='function','Education UI behavioral test API missing');
await api.startEducation();
const state=api.getState();
const pane=document.getElementById('v14EducationPane');
assert(pane,'Education pane was not created');
assert(state.mode==='production'&&state.item?.character==='学',`Production mode did not start with selected Kanji; state=${JSON.stringify(state)}, pane=${pane.innerHTML}`);
const choices=pane.querySelectorAll('.v14-edu-choice');
assert(choices.length===4,'Production must render exactly four MCQ choices');
assert(!pane.querySelector('#v14EduInput'),'Production must not render a text input');
...
api.handleChoice(targetChoice);
assert(pane.innerHTML.includes('✅ پاسخ درست بود'),`Correct Production choice did not render a success result; pane=${pane.innerHTML}; answered=${api.getState().answered}`);
assert(JSON.parse(storage.get('kanji5-v1.2-knowledge')).学.production.attempts===1,'Correct Production answer was not recorded');
await api.startEducation();
const wrongChoice=pane.querySelectorAll('.v14-edu-choice').find(c=>c.dataset.choice!=='学');
assert(wrongChoice,'Wrong Production choice missing');
api.handleChoice(wrongChoice);
assert(pane.innerHTML.includes('❌ پاسخ درست نبود'),'Wrong Production choice did not render an error result');

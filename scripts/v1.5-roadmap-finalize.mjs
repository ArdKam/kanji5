...
// Idempotent: CI may rerun this stage after another commit lands on feature/v1.5.
if(index.includes('function jlptRank(item)')){console.log('v1.5 roadmap finalization already applied.');process.exit(0)}
...
index=index.slice(0,render)+helper+index.slice(render);
index=index.replace('function renderCard(){','function renderCard(){updateUpcomingReviews();');

const startupCandidates=['loadDeck().then(()=>{','(async()=>{'];
let startup=null;for(const marker of startupCandidates){if(index.includes(marker)){startup=marker;break}}
if(!index.includes('setInterval(updateUpcomingReviews,15000)')){
  assert(startup,'startup marker not found');
  index=index.replace(startup,`startUpcomingReviews();\n${startup}`);
}
const startFn=`let upcomingReviewsTimerStarted=false;function startUpcomingReviews(){if(upcomingReviewsTimerStarted)return;upcomingReviewsTimerStarted=true;updateUpcomingReviews();setInterval(updateUpcomingReviews,15000)}\n`;
const fnAnchor=index.indexOf('function ensureUpcomingReviewsUI(){');
assert(fnAnchor>=0,'upcoming helper insertion failed');
index=index.slice(0,fnAnchor)+startFn+index.slice(fnAnchor);

// Keep the live timer single-flight; no per-card interval creation.
index=index.replace('function renderCard(){updateUpcomingReviews();','function renderCard(){updateUpcomingReviews();');

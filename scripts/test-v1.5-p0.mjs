...
const workflow = read('.github/workflows/build-v1.4.yml');
const core = read('v1.4-education-core.js');
const ui = read('v1.4-education-ui.js');

// P0.1 Shell / legacy cleanup.
...
// P0.2 Fresh-install simulation: migration must tolerate an empty store.
class Store {
...
// P0.3 Upgrade simulation from the v1.3 knowledge shape.
...
// P0.4 Offline prerequisites: shell and API caches must include both external providers.
...
assert(sw.includes('kanji5-shell-v37')&&sw.includes('kanji5-api-v14'),'Cache versions were not bumped');

// P0.5 Dependency audit: no ad-hoc package dependency should appear in a static PWA.
...
// P0.6 Core/UI wiring smoke checks.
assert(core.includes("version:'1.5-ready.0'"),'Updated education core is not present');
...
assert(workflow.includes('Run v1.4 education P0 tests')&&workflow.includes('Run v1.4 education P1 tests'),'Existing education CI gates were removed');

...
assert.doesNotMatch(p0, /function\s+applyAttributeRecallOutcome\(/, 'recall outcome mutation must live in the recall core');
...
assert.match(boundary, /await import\('\.\/v1\.4-education-migration\.js'\)/, 'lazy bootstrap must retain migration');
assert.match(boundary, /await import\('\.\/v1\.4-education-core\.js'\)/, 'lazy bootstrap must load education rules');
assert.match(engine, /ensureEducationRuntime/, 'exercise start must request lazy education bootstrap');

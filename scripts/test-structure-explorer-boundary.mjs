...
assert.equal(info.recursive?.[0]?.sourceConfidence,"derived-recursive" in info.recursive?.[0] ? "source-direct" : "source-direct");
...
assert.deepEqual(byRadical.results.map(x=>x.character),["語","議"]);
const direct=await api.getKanjiByComponent("言",false,20);
assert.deepEqual(direct.results.map(x=>x.character),["語","議"]);
const recursive=await api.getKanjiByComponent("言",true,20);
assert.deepEqual(recursive.results.map(x=>x.character),["語","議"]);
const intersection=await api.getKanjiByComponents(["言","義"],true,20);
assert.deepEqual(intersection.results.map(x=>x.character),["議"]);

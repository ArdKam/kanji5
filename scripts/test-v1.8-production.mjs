...
assert.match(ui,/renderChoices\(chooseChoices\(item\)\)/,'production mode must use canonical four-choice Kanji buttons');
...
assert.match(ui,/edu\.mode==='production'.*renderChoices\(chooseChoices\(item\)\)/,'production must render four Kanji choices');

...
if(!source.includes('function fetchContextSentences')){
  ...
  try{
    ...
    url.searchParams.set('lang','jpn');url.searchParams.set('q',ch);url.searchParams.set('trans:lang','eng');url.searchParams.set('trans:is_direct','yes');url.searchParams.set('is_orphan','no');url.searchParams.set('is_unapproved','no');url.searchParams.set('limit','12');
    const response=await fetch(url.toString(),{cache:'force-cache'});if(!response.ok)return[];
    const payload=await response.json(),rows=Array.isArray(payload?.data)?payload.data:[],out=[],seen=new Set();
    for(const row of rows){const text=String(row?.text||'').trim();if(!text||!text.includes(ch)||seen.has(text))continue;seen.add(text);const translations=Array.isArray(row?.translations)?row.translations:[],english=translations.flatMap(group=>Array.isArray(group)?group:[group]).map(t=>String(t?.text||'').trim()).find(Boolean)||'';out.push({id:row?.id||'',text,english});if(out.length>=8)break}
    if(out.length)try{localStorage.setItem(key,JSON.stringify(out))}catch(_){}return out;
  }catch(_){return[]}
}
...
if(!source.includes('sentence:null'))source=source.replace('let edu={item:null,mode:null,word:null,answered:false};','let edu={item:null,mode:null,word:null,sentence:null,answered:false};');
...
const newStart=`async function startEducation(){
  ...
  if(mode==='vocabulary'){words=await fetchWords(item.character);if(!words.length)mode=CORE.chooseBestExercise(item,stats,modes.filter(x=>x!=='vocabulary'),{now:Date.now()})}
  else if(mode==='context'){const sentences=await fetchContextSentences(item.character);sentence=sentences[Math.floor(Math.random()*Math.min(sentences.length,8))]||null;if(!sentence)mode=CORE.chooseBestExercise(item,stats,modes.filter(x=>x!=='context'),{now:Date.now()})}
  writeKnowledge(CORE.ensureEntry(readKnowledge(),item.character,true));edu={item,mode,word:mode==='vocabulary'&&words.length?words[Math.floor(Math.random()*Math.min(words.length,8))]:null,sentence,answered:false};renderEducation();
}
function record(mode,result,wrong=''){`;

...
assert.match(docs,/presentation-only/);assert.match(engine,/__KANJI5_V19_V2_BOUNDARY__/);assert.match(engine,/__KANJI5_EDU_BRIDGE__/);assert.match(app,/<div className="app-shell"/);assert.match(app,/const yieldToBrowser=useCallback/);assert.match(app,/learning-back-identity-visual/);assert.match(app,/setExperience\("dictionary"\);void action\(async\(\)=>\{await clearCustomStudyFilter\(\);await clearTransient\(\)\}\)/);

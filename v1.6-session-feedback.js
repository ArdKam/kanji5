...
setInterval(()=>{exposeAuthoritativeApi();const a=active();if(a){lastResults=normalizeResults(a.modeResults);lastSessionId=a.sessionId;render();return}if(lastResults&&lastSessionId){if(migrateCompletedModeResults(lastSessionId,lastResults))lastResults=null}window.__KANJI5_V16_SKILL_PROFILE__?.update?.();render()},250);

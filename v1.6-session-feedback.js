...
function onResult(e){const d=e?.detail||{};const mode=String(d.mode||'');const correct=Boolean(d.correct);const before=active()?.modeResults||{};const r=normalizeResults(before);if(!MODES.includes(mode))return;r[mode].attempts+=1;if(correct)r[mode].correct+=1;r[mode].lastAt=new Date().toISOString();r[mode].lastCorrect=correct;lastResults=r;lastSessionId=active()?.sessionId||null;record(mode,correct);const token=`${lastSessionId||''}:${mode}:${r[mode].attempts}:${r[mode].correct}`;if(token!==lastRebalanceToken){lastRebalanceToken=token;void rebalance()}render()}

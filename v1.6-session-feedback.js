...
function normalizeResults(value){const out={};for(const mode of MODES){const s=value?.[mode]||{};out[mode]={attempts:Math.max(0,Number(s.attempts)||0),correct:Math.min(Math.max(0,Number(s.attempts)||0),Math.max(0,Number(s.correct)||0)),lastAt:typeof s.lastAt==='string'?s.lastAt:''}}return out}
...
setInterval(()=>{const a=active();if(a){lastResults=normalizeResults(a.modeResults);render();return}if(lastResults){const id=latestCompletedSessionId();if(id){migrateCompletedModeResults(id,lastResults);lastResults=null;render()}}},250);

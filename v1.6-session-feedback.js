...
const KEY_SCHEMA=2,ACTIVE='active',corePromise=import('./v1.6-session-core.js');
...
async function rebalance(){const current=active();if(!current?.plan)return false;const core=await corePromise;const result=core.rebalanceSessionPlan(current.plan,current.remainingModes,current.modeResults,{now:Date.now()});const history=read().filter(x=>x?.status!==ACTIVE);const next=clone(current);next.plan=result.plan;next.remainingModes=result.remaining;next.rebalanceCount=(Number(current.rebalanceCount)||0)+1;history.push(next);return Boolean(state.writeSessionHistory?.(history))}

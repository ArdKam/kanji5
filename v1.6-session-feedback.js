...
function installRuntimeGuards(){const api=window.__KANJI5_V16_SESSION_API__;if(!api||guardedApis.has(api))return false;const originalNext=api.nextMode,originalConsume=api.consumeMode;api.nextMode=available=>authoritativeNextMode(available)||originalNext?.(available)||null;api.consumeMode=mode=>authoritativeConsumeMode(mode)||originalConsume?.(mode)||false;guardedApis.add(api);return true}
...
setInterval(()=>{installRuntimeGuards();const a=active();if(a){lastResults=normalizeResults(a.modeResults);lastSessionId=a.sessionId;render();return}if(lastResults&&lastSessionId){if(migrateCompletedModeResults(lastSessionId,lastResults))lastResults=null}render()},250);
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{installRuntimeGuards();render()},{once:true});else{installRuntimeGuards();render()}
})();

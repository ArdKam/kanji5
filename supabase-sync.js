...
(() => {
  ...
  const mergeState = (local, remote) => {
    ...
    merged.cards = {};
    for (const id of ids) {
      const lc = local.cards?.[id];
      const rc = remote.cards?.[id];
      ...
      for (const mode of ["meaning", "reading"]) {
        ...
        if (!l) out[ch][mode] = r;
          ...
          out[ch][mode] = {
            ...
            lastAt: String(l.lastAt || "") > String(r.lastAt || "") ? l.lastAt : r.lastAt
          };
        }
      }
  ...
  const readMeta = () => safeJSON(localStorage.getItem(SYNC_META_KEY), {});
  const writeMeta = value => { try { localStorage.setItem(SYNC_META_KEY, json(value)); } catch (_) {} };
  const setStatus = (text, tone = "") => {
    const el = byId("syncStatusText");
  ...
  async function push(local, remote) {
    ...
    syncing = true;
    try {
      const merged = { state: mergeState(local, remote), knowledge: mergeKnowledge(local.knowledge, remote?.knowledge), deckVersion: local.deckVersion || remote?.deckVersion || null };
      writeLocal(merged);
      const payload = { ...merged, clientUpdatedAt: new Date().toISOString() };
      ...
      lastHash = hash(merged);
      writeMeta({ userId: user.id, syncedAt: payload.clientUpdatedAt });
  ...
  async function pullAndMerge() {
    ...
    try {
      ...
      if (h !== hash(local)) {
        ...
        window.dispatchEvent(new CustomEvent("kanji5-sync-applied"));
        setTimeout(() => location.reload(), 250);
      } else {
        lastHash = h;
        setStatus("همگام است", "ok");
      }
      if (h !== hash(remote)) await push(merged, remote);
    } catch (e) {
  ...
  function ensureUI() {
    ...
    button.id = "syncBtn";
    button.setAttribute("aria-label", "حساب و همگام‌سازی");
  ...
  function openAuth() {
    const dialog = byId("syncDialog");
    const content = byId("syncContent");
    ...
    if (user) {
      ...
      byId("googleLogin").addEventListener("click", async () => {
        ...
        if (error) byId("syncMsg").textContent = error.message;
      });
  ...
  async function emailAuth(signup) {
    const email = byId("syncEmail")?.value.trim();
    const password = byId("syncPassword")?.value || "";
    const msg = byId("syncMsg");
  ...
  async function boot() {
    ...
    try {
      ...
      c.auth.onAuthStateChange(async (_event, session) => {
        ...
        if (user) { await pullAndMerge(); clearInterval(pollTimer); pollTimer = setInterval(pullAndMerge, POLL_MS); }
        else { clearInterval(pollTimer); setStatus("وارد نشده"); }
      });

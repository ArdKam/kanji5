...
(() => {
  ...
  async function replaceRemote(payload, expectedUpdatedAt = null) {
    ...
    const rowPayload = { ...stablePayload(payload), clientUpdatedAt: now };
    if (expectedUpdatedAt) {
      const { data, error } = await client.from("user_learning_state")
  ...
  async function syncOnce() {
    ...
    if (!remoteRow?.payload) {
      ...
      writeLocal(stablePayload(local), result.updatedAt);
      setStatus("همگام شد", "ok");
      return { retry: false };
    }

    const merged = mergeSyncPayload(local, remoteRow.payload);
    ...
    if (mergedHash !== localHash) {
      setStatus("داده‌های ادغام‌شده دریافت شد", "ok");
      setTimeout(() => location.reload(), 250);
    } else setStatus("همگام شد", "ok");
    return { retry: false };
  }
    ...
    return withSyncLock(async () => {
      for (let attempt = 1; attempt <= MAX_SYNC_ATTEMPTS; attempt += 1) {
        try {
          ...
          console.warn("Kanji 5 sync attempt failed", e);
    ...
    try {
      ...
      if (user) {
        ...
        window.addEventListener("storage", event => {
          if ([STORAGE_KEY, CARDS_STORAGE_KEY, REVIEWS_STORAGE_KEY, KNOWLEDGE_KEY].includes(event.key)) pullAndMerge();
        });

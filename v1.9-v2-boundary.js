...
async function snapshot(revision=publishRevision){const core=await load(),session=activeSession()||completedSession();const viewModel=core.buildBoundarySnapshot({session,exercise,feedback,learner:learner(),recentOutcomes:recentOutcomes(),adaptiveReason});return Object.freeze({...viewModel,viewRevision:Number(revision)||0})}
async function publish(){const revision=++publishRevision;const viewModel=await snapshot(revision);if(revision!==publishRevision)return null;window.__KANJI5_V19_V2_LAST_SNAPSHOT__=viewModel;document.dispatchEvent(new CustomEvent('kanji5:v1.9-v2-view-models',{detail:viewModel}));return viewModel}

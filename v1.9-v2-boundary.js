...
function kanjiMastery(character,knowledge,model){
  ...
  for(const mode of modes){
    ...
    const value=Number(projected.mastery);
    if(attempts>0) total+=Number.isFinite(value)?Math.max(0,Math.min(1,value)):Math.max(0,Math.min(1,(Number(projected.correct??raw.correct)||0)+1)/(attempts+2));
  }

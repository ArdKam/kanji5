...
test('React exercise path can start and expose a boundary-backed exercise',async({page})=>{
  ...
  await expect.poll(async()=>page.evaluate(()=>window.__KANJI5_V19_V2_BOUNDARY__?.snapshot?.().then(s=>s.exercise?.mode))).toMatch(/meaning|reading|production|vocabulary|context/);
});
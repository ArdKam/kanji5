...
test('v2 P3 supports keyboard focus, live feedback semantics, mobile layout, and reduced motion',async({page})=>{
  ...
  await page.evaluate(async()=>{
    ...
    await b.setFeedback({mode:'production',outcome:'wrong',correct:false,score:0,graderVersion:'1.9.0-production',reason:'recent failure'});
  });
  ...
  const hasInputFocus=await page.evaluate(()=>document.activeElement?.id==='v2AnswerInput');

(() => {
  ...
  function choosePrompt(character) {
    ...
    const last = loadKnowledge()[character]?.lastPrompt;
    return last === "meaning" ? "reading" : "meaning";
  }
  ...
  async function loadDeckIndex() {
    ...
    deckLoadPromise = (async () => {
      try {
        ...
        const deck = Array.isArray(cached) ? cached : null;
        if (deck?.length) {
          deckIndex = new Map(deck.map(item => [item.character, item]));
          return deckIndex;
        }
  ...
  function addRecallFeedback(gate, correct) {
    if (correct == null) return;
    const feedback = document.createElement("div");
    feedback.style.cssText = "margin-top:8px;font-size:13px;color:var(--muted);text-align:center";
    feedback.textContent = correct ? "تلاش بازیابی ثبت شد." : "تلاش بازیابی ثبت شد و این بخش برای مرور بعدی ضعیف‌تر در نظر گرفته می‌شود.";

...
function VocabularyExamples({ character: kanjiCharacter, language, catalog, onSelectKanji }: { character: string; language: Language; catalog: KanjiCatalogItem[]; onSelectKanji: (item: KanjiCatalogItem) => void }) {
...
function ComponentLearningPath({
  ...
  character: string;
  nodes: import("./engine").ComponentNode[];
  ...
  const renderNode = (node: import("./engine").ComponentNode, depth: number, path: Set<string>) => {
    if (depth > 3 || path.has(node.glyph)) return null;
    const item = catalogByCharacter.get(node.glyph);
    const mastery = item ? Math.round(Math.max(0, Math.min(1, Number(item.mastery) || 0)) * 100) : null;
    const nextPath = new Set(path);
    nextPath.add(node.glyph);
    const children = (node.children ?? []).filter(child => child.glyph !== node.glyph);

    return (
      <div className={`component-learning-path-node depth-${depth}`} key={node.glyph + "-" + depth}>
        <div className="component-learning-path-item">
          <div className="component-learning-path-main">
            {item ? (
              <button className="component-learning-path-kanji" type="button" lang="ja" onClick={() => onSelectKanji(item)} title={t("lookupKanji", language)}>
                {node.glyph}
              </button>
            ) : (
              <span className="component-learning-path-kanji is-static" lang="ja">{node.glyph}</span>
            )}
            <div className="component-learning-path-copy">
              ...
              {children.length ? <span lang="ja">{children.map(child => child.glyph).join(" + ")}</span> : <span>{t("componentLearningPathLeaf", language)}</span>}
            </div>
          </div>
          {mastery !== null ? (
  ...
  return (
    <section className="component-learning-path" aria-label={language === "fa" ? "نقشهٔ یادگیری اجزای کانجی" : "Kanji component learning graph"}>
      ...
      <div className="component-learning-path-list" role="list">
        {nodes.map(node => renderNode(node, 0, new Set<string>()))}
      </div>
    </section>
  );
}
  ...
  return (
    <dialog open className="dialog dictionary-card-dialog" aria-label={t("dictionary", language)}>
      ...
      <div className="dictionary-card">
        ...
        {componentInfo?.available && componentInfo.components.length ? (
          <ComponentLearningPath character={item.character} nodes={componentInfo.recursive ?? componentInfo.components.map(glyph => ({ glyph, relation: "leaf", sourceConfidence: "source-direct" as const, children: [] }))} catalog={catalog} language={language} onSelectKanji={onSelectKanji} />
        ) : null}

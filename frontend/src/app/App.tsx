...
function Insights({snapshot}:{snapshot:Snapshot}){return <details className="insights"><summary>{t("sessionDetails")}</summary><div className="insights-grid">
  <Panel title={t("masteryOverview")}><div className="mastery-grid">{skillKeys.map(k=>{const s=snapshot.learner?.attributes?.[k]??{};const value=pct(s.confidence??s.accuracy);const trend=Number(s.momentum??0);return <div className="mastery-row" key={k}><div className="mastery-label"><span>{skillLabel(k)}</span><strong>{fa(value)}%</strong></div><div className="mastery-track" role="progressbar" aria-label={skillLabel(k)+" — "+t("modelConfidence")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={value}><span style={{width:value+"%"}} /></div><div className="mastery-state"><span>{stateLabel(s.state??"")}</span><span className={trend>0.02?"trend-up":trend<-0.02?"trend-down":"trend-flat"}>{trend>0.02?"↑":trend<-0.02?"↓":"→"} {fa(Math.round(Math.abs(trend)*100))}%</span></div></div>)}</div><p className="mastery-footnote">{t("modelConfidence")}</p></Panel>
  <Panel title={t("sevenDayActivity")}><div className="activity-chart" aria-label={t("sevenDayActivity")}>{(snapshot.stats?.last7??[]).map((d,i)=>{const count=Math.max(0,Number(d.count)||0);const max=Math.max(1,...(snapshot.stats?.last7??[]).map(x=>Math.max(0,Number(x.count)||0)));const height=Math.max(6,Math.round(count/max*100));return <div className="activity-bar-wrap" key={(d.label??"")+(i)}><div className="activity-count">{fa(count)}</div><div className="activity-bar-track" aria-hidden="true"><span style={{height:height+"%"}} /></div><div className="activity-label">{text(d.label,"—")}</div></div>})}</div></Panel>

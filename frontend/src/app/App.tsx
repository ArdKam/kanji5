...
function App(){
  ...
  return <div className="app-shell">
    ...
    <header className="header"><div className="header-brand"><p className="eyebrow red">常用漢字 · 2,136</p><h1>{t("title")}</h1><p className="subtitle">{t("subtitle")}</p></div>
      <div className="header-actions">{hasSessionProgress?<div className="session-progress"><Progress value={progress} label={t("sessionProgress")}/><span>{fa((snapshot?.session?.plannedTotal??0)-(snapshot?.session?.remainingTotal??0))} {language==="fa"?"از":"of"} {fa(snapshot?.session?.plannedTotal??0)}</span></div>:null}
    ...
    <SettingsDialog open={settingsOpen} snapshot={snapshot??{}} busy={busy} language={language} onLanguageChange={changeLanguage} onClose={()=>setSettingsOpen(false)} onSave={s=>void action(async()=>{await updateSettings(s);setSettingsOpen(false)})} onReset={()=>void action(async()=>{resetProgress()})}/>
  </div>
}

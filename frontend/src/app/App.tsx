...
function Learning({card,mnemonics,onReveal,onRate}:{card:NonNullable<Snapshot["learning"]>;mnemonics:Record<string,string>;onReveal:()=>void;onRate:(r:Rating)=>void}){
  ...
  <div className="mnemonic-head"><h3>{t("mnemonic")}</h3>{mnemonic&& !mnemonicEditing?<button className="link-button" type="button" onClick={()=>setMnemonicEditing(true)}>{getLanguage()==="fa"?"ویرایش":"Edit"}</button>:null}</div>
  {mnemonic && !mnemonicEditing?<><div className="mnemonic-body">{mnemonic}</div><button className="mnemonic-delete" type="button" disabled={mnemonicSaving} onClick={()=>{setMnemonic("");setMnemonicEditing(true)}}>{t("deleteMnemonic")}</button></>:<div className="mnemonic-editor"><textarea autoFocus value={mnemonic} maxLength={500} onChange={e=>setMnemonic(e.target.value)} placeholder={t("mnemonicPlaceholder")} aria-label={t("mnemonicPlaceholder")}/><div className="mnemonic-actions"><button className="button primary" type="button" disabled={mnemonicSaving} onClick={()=>void persistMnemonic()}>{t("saveMnemonic")}</button>{mnemonic?<button className="button secondary" type="button" disabled={mnemonicSaving} onClick={()=>{setMnemonic(mnemonics[mnemonicCharacter]??"");setMnemonicEditing(false)}}>{t("close")}</button>:null}</div></div>}

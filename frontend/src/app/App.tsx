...
function App(){
  ...
  useEffect(()=>{let mounted=true;void startLearningExperience().catch(()=>{});const listener=(e:Event)=>{const d=(e as CustomEvent<Snapshot>).detail;if(mounted&&d)setSnapshot(d)};void refresh().then(()=>{document.addEventListener("kanji5:v1.9-v2-view-models",listener);document.addEventListener("kanji5:v1.9-sync-applied",listener)}).catch(e=>{if(mounted)setError(e instanceof Error?e.message:t("learningCoreError"))});return()=>{mounted=false;document.removeEventListener("kanji5:v1.9-v2-view-models",listener);document.removeEventListener("kanji5:v1.9-sync-applied",listener)}},[refresh]);
  ...
  return <div className="app-shell">
    ...
    <nav className="experience-nav" aria-label={t("learningPath",language)}><button className={"experience-tab "+(experience==="review"?"active":"")} type="button" aria-current={experience==="review"?"page":undefined} onClick={()=>{setExperience("review");void startLearningExperience().then(clearTransient).then(refresh).catch(e=>setError(e instanceof Error?e.message:t("learningCoreError")))}}>{t("learning",language)}</button><button className={"experience-tab "+(experience==="practice"?"active":"")} type="button" aria-current={experience==="practice"?"page":undefined} onClick={()=>{setExperience("practice");void startExercise().then(refresh).catch(e=>setError(e instanceof Error?e.message:t("learningCoreError")))}}>{t("activeRecall",language)}</button><button className={"experience-tab "+(experience==="dictionary"?"active":"")} type="button" aria-current={experience==="dictionary"?"page":undefined} onClick={()=>{setExperience("dictionary");void clearCustomStudyFilter().then(clearTransient).then(refresh).catch(e=>setError(e instanceof Error?e.message:t("learningCoreError")))}}>{t("dictionary",language)}</button></nav><main id="primary-content" className="content mobile-study-flow">

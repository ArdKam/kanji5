...
function App(){
  ...
  return <div className="app-shell">
    ...
    <header className="header"><div><p className="eyebrow red">常用漢字 · 2,136</p><h1>کانجی ۵</h1><p className="subtitle">Sumi Play presentation · v1.9 engine authoritative</p></div>
      ...
      <button className="button secondary" type="button" disabled={busy} onClick={()=>void action(async()=>{await startExercise();setExperience("practice")})}>تمرین آموزشی</button><button className="button secondary" type="button" disabled={busy} onClick={()=>setStatsOpen(true)}>آمار</button><button className="button secondary" type="button" disabled={busy} onClick={()=>setSettingsOpen(true)}>تنظیمات</button></div>
    </header>
    <nav className="experience-nav" aria-label="مسیر یادگیری"><button className={"experience-tab "+(experience==="review"?"active":"")} type="button" aria-current={experience==="review"?"page":undefined} disabled={busy} onClick={()=>void action(async()=>{await clearTransient();setExperience("review")})}>مرور</button><button className={"experience-tab "+(experience==="practice"?"active":"")} type="button" aria-current={experience==="practice"?"page":undefined} disabled={busy} onClick={()=>void action(async()=>{await startExercise();setExperience("practice")})}>تمرین</button></nav><main id="primary-content" className="content">

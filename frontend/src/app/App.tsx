...
function App(){
  ...
  return <div className="app-shell">
    ...
    <header className="header"><div className="header-brand"><div className="language-switcher" role="group" aria-label={t("language")}><button className={"language-button "+(language==="fa"?"active":"")} type="button" aria-pressed={language==="fa"} onClick={()=>changeLanguage("fa")}>{t("persian")}</button><button className={"language-button "+(language==="en"?"active":"")} type="button" aria-pressed={language==="en"} onClick={()=>changeLanguage("en")}>{t("english")}</button></div><p className="eyebrow red">常用漢字 · 2,136</p><h1>{t("title")}</h1><p className="subtitle">{t("subtitle")}</p></div>
      <div className="header-actions"><div className="session-progress"><Progress value={progress} label={t("sessionProgress")}/><span>{fa((snapshot?.session?.plannedTotal??0)-(snapshot?.session?.remainingTotal??0))} {language==="fa"?"از":"of"} {fa(snapshot?.session?.plannedTotal??0)}</span></div>

...
function App(){
  ...
  const progress=pct(snapshot?.session?.completionFraction);const showExercise=exerciseMode||Boolean(snapshot?.exercise?.mode);
  ...
  return <div className="app-shell">
    ...
    <header className="header"><div><p className="eyebrow red">常用漢字 · 2,136</p><h1>کانجی ۵</h1><p className="subtitle">Sumi Play presentation · v1.9 engine authoritative</p></div>
      ...
      <button className="button secondary" type="button" disabled={busy} onClick={()=>void action(async()=>{await startExercise();setExerciseMode(true)})}>تمرین آموزشی</button><button className="button secondary" type="button" disabled={busy} onClick={()=>setStatsOpen(true)}>آمار</button><button className="button secondary" type="button" disabled={busy} onClick={()=>setSettingsOpen(true)}>تنظیمات</button></div>
    </header>
    <main id="primary-content" className="content">
      ...
      {showExercise?<><Exercise snapshot={snapshot??{}} busy={busy} onSubmit={v=>void action(()=>submitExercise(v))} onDontKnow={()=>void action(dontKnow)} onRetry={()=>void action(retryExercise)} onNext={()=>void action(nextExercise)} onBack={()=>void action(async()=>{await clearTransient();setExerciseMode(false)})}/><Feedback snapshot={snapshot??{}}/></>:snapshot?.learning?.active?<Learning card={snapshot.learning} onReveal={()=>void action(revealLearning)} onRate={r=>void action(async()=>{await rateLearning(r);setExerciseMode(false)})}/>:<section className="surface empty-state"><h2>جلسه‌ای برای نمایش وجود ندارد</h2><p>برای شروع تمرین آموزشی از دکمهٔ بالای صفحه استفاده کن.</p><button className="button primary" type="button" onClick={()=>void action(async()=>{await startExercise();setExerciseMode(true)})}>شروع تمرین</button></section>}

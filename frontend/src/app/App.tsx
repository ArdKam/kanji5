  ...
  return <section className={"surface card learning-card "+(revealed?"is-revealed":"")} aria-label={t("learningCard")}>
    <div className="learning-card-flip" aria-live="polite">
      ...
      <div className="learning-card-face learning-card-back" aria-hidden={!revealed}>
        ...
        {card.examples?.length?<div className="examples compact-examples"><h3>{t("vocabularyExamples")}</h3>{card.examples.slice(0,3).map((e,i)=><div className="example-row" key={(e.word??"")+"-"+i} lang="ja"><span>{[e.word,e.reading].filter(Boolean).join(" · ")}</span>{e.reading?<Audio value={e.reading} label="t("playWordPronunciation")"/>:null}</div>)}</div>:null}
...
function Exercise({snapshot,busy,onSubmit,onDontKnow,onRetry,onNext,onBack}:{snapshot:Snapshot;busy:boolean;onSubmit:(v:string)=>void;onDontKnow:()=>void;onRetry:()=>void;onNext:()=>void;onBack:()=>void}){
  ...
  return <section id="exercise" className="surface card" tabIndex={-1}><div className="card-topline"><span className="badge">{text(ex.mode,t("currentExercise"))}</span><span>{t("activeRecallLabel")}</span></div><h2>{t("currentExercise")}</h2><p className="prompt">{localizeDynamic(ex.prompt,getLanguage(),t("exerciseReady"))}</p>
    {!ex.mode?<div className="empty-state">هنوز تمرینی آماده نیست.</div>:<><Stimulus ex={ex}/>{production?<div className="production-grid">{choices.map(c=><button className="button production-choice" type="button" key={c} lang="ja" disabled={busy} onClick={()=>onSubmit(c)}>{c}</button>)}</div>:<label className="answer-area"><span>{t("answerYourself")}</span><input autoFocus value={answer} disabled={busy} onChange={e=>setAnswer(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();onSubmit(answer)}}} placeholder={localizeDynamic(ex.stimulus?.inputPlaceholder,getLanguage(),t("answerPlaceholder"))}/></label>}
...
function Insights({snapshot}:{snapshot:Snapshot}){return <details className="insights"><summary>{t("sessionDetails")}</summary><div className="insights-grid">
  <Panel title={t("learnerSkills")}>{Object.keys(skills).map(k=>{const s=snapshot.learner?.attributes?.[k]??{};return <p className="row" key={k}><span>{skillLabel(k)}</span><strong>{stateLabel(s.state??"")} · {t("recent")} {fa(pct(s.recentAccuracy))}%</strong></p>})}</Panel>
  <Panel title={t("adaptiveFocus")}><p className="row"><span>مهارت</span><strong>{skillLabel(snapshot.adaptiveReason?.mode??"")}</strong></p><p className="row"><span>عمل</span><strong>{actionLabel(snapshot.adaptiveReason?.action??"")}</strong></p>{(snapshot.adaptiveReason?.reasons??[]).filter(Boolean).map((r,i)=><p className="reason" key={r+"-"+i}>{r}</p>)}</Panel>
  <Panel title={t("sessionSummary")}><p className="row"><span>{t("attempts")}</span><strong>{fa(snapshot.sessionSummary?.attempts??0)}</strong></p><p className="row"><span>{t("right")}</span><strong>{fa(snapshot.sessionSummary?.correct??0)}</strong></p><p className="row"><span>{t("accuracy")}</span><strong>{fa(pct(snapshot.sessionSummary?.accuracy))}٪</strong></p></Panel>
...
function SettingsDialog({open,snapshot,busy,onClose,onSave,onReset}:{open:boolean;snapshot:Snapshot;busy:boolean;onClose:()=>void;onSave:(s:Settings)=>void;onReset:()=>void}){
  ...
  return open?<dialog open className="dialog" aria-labelledby="settings-title"><button className="dialog-close" type="button" aria-label={t("close")} onClick={onClose}>×</button><h2 id="settings-title">{t("settingsTitle")}</h2><form className="settings-form" onSubmit={e=>{e.preventDefault();onSave(draft)}}>
...
function App(){
  ...
  return <div className="app-shell">
    ...
    <header className="header"><div><p className="eyebrow red">常用漢字 · 2,136</p><h1>{t("title")}</h1><p className="subtitle">{t("subtitle")}</p></div>
      <div className="header-actions"><div className="language-switcher" role="group" aria-label={t("language")}><button className={"language-button "+(language==="fa"?"active":"")} type="button" aria-pressed={language==="fa"} onClick={()=>changeLanguage("fa")}>{t("persian")}</button><button className={"language-button "+(language==="en"?"active":"")} type="button" aria-pressed={language==="en"} onClick={()=>changeLanguage("en")}>{t("english")}</button></div><div className="session-progress"><Progress value={progress} label={t("sessionProgress")}/><span>{fa((snapshot?.session?.plannedTotal??0)-(snapshot?.session?.remainingTotal??0))} از {fa(snapshot?.session?.plannedTotal??0)}</span></div>
      <button className="button secondary" type="button" disabled={busy} onClick={()=>setStatsOpen(true)}>{t("stats")}</button><button className="button secondary" type="button" disabled={busy} onClick={()=>setSettingsOpen(true)}>{t("settings")}</button></div>
    </header>
    <nav className="experience-nav" aria-label={t("learningPath")}><button className={"experience-tab "+(experience==="review"?"active":"")} type="button" aria-current={experience==="review"?"page":undefined} disabled={busy} onClick={()=>void action(async()=>{await startLearningExperience();await clearTransient();setExperience("review")})}>{t("learning")}</button><button className={"experience-tab "+(experience==="practice"?"active":"")} type="button" aria-current={experience==="practice"?"page":undefined} disabled={busy} onClick={()=>void action(async()=>{await startExercise();setExperience("practice")})}>{t("activeRecall")}</button></nav><main id="primary-content" className="content">
      ...
      {!showExercise&&snapshot?.upcomingReviews?.length?<details className="surface upcoming"><summary>{t("upcomingReviews")}</summary><div className="upcoming-body">{snapshot.upcomingReviews.map(r=><div className="upcoming-row" key={r.character+r.dueAt}><strong lang="ja">{r.character}</strong><span>{new Date(r.dueAt).toLocaleString("fa-IR",{dateStyle:"medium",timeStyle:"short"})}</span></div>)}</div></details>:null}

...
skip.className='v2-skip-link';skip.href='#v2Exercise';skip.textContent='Skip to current exercise';skip.addEventListener('click',event=>{const target=document.getElementById('v2Exercise');if(!target)return;event.preventDefault();target.scrollIntoView({block:'start'});requestAnimationFrame(()=>target.focus({preventScroll:true}));});root.appendChild(skip);

...
skip.className='v2-skip-link';skip.href='#v2Exercise';skip.textContent='Skip to current exercise';const focusExercise=event=>{const target=document.getElementById('v2Exercise');if(!target)return;event.preventDefault();target.focus({preventScroll:true});requestAnimationFrame(()=>target.focus({preventScroll:true}));};skip.addEventListener('click',focusExercise);skip.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){focusExercise(event);}});root.appendChild(skip);
...
function render(snapshot) {
  ...
  if (ex?.mode) {
    ...
    submit.addEventListener('click',async()=>{
      ...
      const latest=await window.__KANJI5_V19_V2_BOUNDARY__?.snapshot?.();
  ...
  if (snapshot?.feedback?.state==='pending_retry') {
    ...
    retry.addEventListener('click',async()=>{const bridge=window.__KANJI5_EDU_BRIDGE__;if(!bridge?.retry)return;retry.disabled=true;await bridge.retry();const latest=await window.__KANJI5_V19_V2_BOUNDARY__?.snapshot?.();if(latest)render(latest)});

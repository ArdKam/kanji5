...
const KEY='v16SkillProfile',SCHEMA=2,RECENT_WINDOW=3,EVIDENCE_WINDOW=8;
...
function outcomeEvidence(rows,mode){const source=rows.slice(-EVIDENCE_WINDOW).map(row=>row.modeResults?.[mode]).filter(s=>s&&typeof s.lastCorrect==='boolean');let recentErrors=0,recoveryOpportunities=0,recoveries=0;for(let i=0;i<source.length;i+=1){if(source[i].lastCorrect===false){recentErrors+=1;const next=source[i+1];if(next){recoveryOpportunities+=1;if(next.lastCorrect===true)recoveries+=1}}}return{recentErrors,recoveryOpportunities,recoveries,recoveryRate:recoveryOpportunities?recoveries/recoveryOpportunities:0}}
function aggregateRecent(rows){return aggregate(rows.slice(-RECENT_WINDOW))}
function buildProfile(rows){const all=aggregate(rows),recent=aggregateRecent(rows),skills={};for(const m of MODES){const base=all[m],window=recent[m],momentum=base.attempts&&window.attempts?Math.max(-1,Math.min(1,(window.accuracy-base.accuracy)/100)):0,evidence=outcomeEvidence(rows,m);skills[m]={...base,recentAttempts:window.attempts,recentCorrect:window.correct,recentAccuracy:window.accuracy,momentum,...evidence}}return{schemaVersion:SCHEMA,updatedAt:new Date().toISOString(),sessions:rows.length,skills}}

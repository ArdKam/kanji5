...
const STATUS='skill-profile',SCHEMA=1;
...
function read(){return(state.readSessionHistory?.()||[]).find(x=>x?.status===STATUS&&Number(x.schemaVersion)===SCHEMA)||null}
function update(){const rows=completed(),skills=aggregate(rows),history=state.readSessionHistory?.()||[],next={schemaVersion:SCHEMA,status:STATUS,updatedAt:new Date().toISOString(),sessions:rows.length,skills};const old=history.findIndex(x=>x?.status===STATUS);if(old>=0)history[old]=next;else history.push(next);if(history.length>state.SESSION_HISTORY_LIMIT)history.splice(0,history.length-state.SESSION_HISTORY_LIMIT);return Boolean(state.writeSessionHistory?.(history))}

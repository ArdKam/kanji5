...
let fingerprint='';
function update(){const rows=completed();if(!rows.length){render();return false}const source=rows.map(x=>`${x.sessionId||''}:${x.endedAt||''}:${JSON.stringify(x.modeResults||{})}`).join('|');if(source===fingerprint){render();return true}fingerprint=source;const history=state.readSessionHistory?.()||[],next={schemaVersion:SCHEMA,status:STATUS,updatedAt:new Date().toISOString(),sessions:rows.length,skills:aggregate(rows)};const old=history.findIndex(x=>x?.status===STATUS);if(old>=0)history[old]=next;else history.push(next);if(history.length>state.SESSION_HISTORY_LIMIT)history.splice(0,history.length-state.SESSION_HISTORY_LIMIT);const ok=Boolean(state.writeSessionHistory?.(history));render();return ok}

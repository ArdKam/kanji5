...
export function buildSessionPlan(knowledge={},options={}){
 ...
 const entries=MODES.map(mode=>{let attempts=0,correct=0,lastAt='';for(const entry of Object.values(knowledge||{})){const s=safeStats(entry?.[mode]);attempts+=s.attempts;correct+=s.correct;if(s.lastAt>lastAt)lastAt=s.lastAt}const mastery=(correct+1)/(attempts+2),weakness=1-mastery,profileRow=hasProfile?profileStats(profile,mode):null,profileBoost=profileRow?(profileRow.weakness*.65*(.25+.75*profileRow.confidence)):0,urgency=weakness*1.7+profileBoost+recencyBoost(lastAt,now)*.15+(attempts===0?.45:0);return{mode,label:LABELS[mode],attempts,correct,mastery,accuracy:attempts?correct/attempts*100:0,profileAttempts:profileRow?.attempts||0,profileAccuracy:profileRow?.accuracy||0,profileMastery:profileRow?.mastery||0,score:Math.max(.01,urgency)}});
 ...
 return{version:1,target,generatedAt:new Date(now).toISOString(),profileAware:hasProfile,modes:shares.map(item=>({mode:item.mode,label:item.label,plannedCount:item.plannedCount,share:item.share,mastery:item.mastery,accuracy:item.accuracy,attempts:item.attempts,profileMastery:item.profileMastery,profileAccuracy:item.profileAccuracy,profileAttempts:item.profileAttempts,score:item.score})),priority:shares.filter(item=>item.plannedCount>0).map(item=>item.mode)};
}

...
function profileStats(profile,mode){const s=safeStats(profile.skills?.[mode]);const confidence=Math.min(1,s.attempts/20);const momentum=Math.max(-1,Math.min(1,Number(profile.skills?.[mode]?.momentum)||0));return{...s,confidence,weakness:1-s.mastery,momentum,recentAccuracy:Number(profile.skills?.[mode]?.recentAccuracy)||0,recentAttempts:Math.max(0,Number(profile.skills?.[mode]?.recentAttempts)||0)}}
...
export function buildSessionPlan(knowledge={},options={}){
 ...
 const entries=MODES.map(mode=>{let attempts=0,correct=0,lastAt='';for(const entry of Object.values(knowledge||{})){const s=safeStats(entry?.[mode]);attempts+=s.attempts;correct+=s.correct;if(s.lastAt>lastAt)lastAt=s.lastAt}const mastery=(correct+1)/(attempts+2),weakness=1-mastery,profileRow=hasProfile?profileStats(profile,mode):null,profileBoost=profileRow?(profileRow.weakness*.65*(.25+.75*profileRow.confidence)):0,trendBoost=profileRow?Math.max(0,-profileRow.momentum)*.3-Math.max(0,profileRow.momentum)*.08:0,urgency=weakness*1.7+profileBoost+trendBoost+recencyBoost(lastAt,now)*.15+(attempts===0?.45:0);return{mode,label:LABELS[mode],attempts,correct,mastery,accuracy:attempts?correct/attempts*100:0,profileAttempts:profileRow?.attempts||0,profileAccuracy:profileRow?.accuracy||0,profileMastery:profileRow?.mastery||0,profileRecentAttempts:profileRow?.recentAttempts||0,profileRecentAccuracy:profileRow?.recentAccuracy||0,profileMomentum:profileRow?.momentum||0,profileTrendBoost:trendBoost,score:Math.max(.01,urgency)}});
 ...
 return{version:2,target,generatedAt:new Date(now).toISOString(),profileAware:hasProfile,modes:shares.map(item=>({mode:item.mode,label:item.label,plannedCount:item.plannedCount,share:item.share,mastery:item.mastery,accuracy:item.accuracy,attempts:item.attempts,profileMastery:item.profileMastery,profileAccuracy:item.profileAccuracy,profileAttempts:item.profileAttempts,profileRecentAttempts:item.profileRecentAttempts,profileRecentAccuracy:item.profileRecentAccuracy,profileMomentum:item.profileMomentum,profileTrendBoost:item.profileTrendBoost,score:item.score})),priority:shares.filter(item=>item.plannedCount>0).map(item=>item.mode)};
}
